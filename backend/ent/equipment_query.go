...
type EquipmentQuery struct {
	...
	// eager-loading edges.
	withOrderEquipmentt *OrderQuery
...
// QueryOrderEquipmentt chains the current query on the order_equipmentt edge.
func (eq *EquipmentQuery) QueryOrderEquipmentt() *OrderQuery {
	query := &OrderQuery{config: eq.config}
...
//  WithOrderEquipmentt tells the query-builder to eager-loads the nodes that are connected to
// the "order_equipmentt" edge. The optional arguments used to configure the query builder of the edge.
func (eq *EquipmentQuery) WithOrderEquipmentt(opts ...func(*OrderQuery)) *EquipmentQuery {
	query := &OrderQuery{config: eq.config}
	for _, opt := range opts {
		opt(query)
	}
	eq.withOrderEquipmentt = query
	return eq
}
...
func (eq *EquipmentQuery) sqlAll(ctx context.Context) ([]*Equipment, error) {
	...
	if query := eq.withOrderEquipmentt; query != nil {
		...
		query.Where(predicate.Order(func(s *sql.Selector) {
			s.Where(sql.InValues(equipment.OrderEquipmenttColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.equipment_order_equipmentt
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "equipment_order_equipmentt" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "equipment_order_equipmentt" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.OrderEquipmentt = append(node.Edges.OrderEquipmentt, n)
		}
	}
