...
// QueryOrderCompanyy chains the current query on the order_companyy edge.
func (cq *CompanyQuery) QueryOrderCompanyy() *OrderQuery {
...
//  WithOrderCompanyy tells the query-builder to eager-loads the nodes that are connected to
// the "order_companyy" edge. The optional arguments used to configure the query builder of the edge.
func (cq *CompanyQuery) WithOrderCompanyy(opts ...func(*OrderQuery)) *CompanyQuery {
	query := &OrderQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withOrderCompanyy = query
	return cq
}
...
func (cq *CompanyQuery) sqlAll(ctx context.Context) ([]*Company, error) {
	...
	if query := cq.withOrderCompanyy; query != nil {
		...
		query.Where(predicate.Order(func(s *sql.Selector) {
			s.Where(sql.InValues(company.OrderCompanyyColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.company_order_companyy
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "company_order_companyy" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "company_order_companyy" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.OrderCompanyy = append(node.Edges.OrderCompanyy, n)
		}
	}
