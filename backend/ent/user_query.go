...
// QueryOrderUserr chains the current query on the order_userr edge.
func (uq *UserQuery) QueryOrderUserr() *OrderQuery {
...
//  WithOrderUserr tells the query-builder to eager-loads the nodes that are connected to
// the "order_userr" edge. The optional arguments used to configure the query builder of the edge.
func (uq *UserQuery) WithOrderUserr(opts ...func(*OrderQuery)) *UserQuery {
	query := &OrderQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withOrderUserr = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withOrderUserr; query != nil {
		...
		query.Where(predicate.Order(func(s *sql.Selector) {
			s.Where(sql.InValues(user.OrderUserrColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_order_userr
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_order_userr" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_order_userr" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.OrderUserr = append(node.Edges.OrderUserr, n)
		}
	}
