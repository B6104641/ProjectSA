...
func (oq *OrderQuery) QueryEquipment() *EquipmentQuery {
	query := &EquipmentQuery{config: oq.config}
...
func (oq *OrderQuery) WithEquipment(opts ...func(*EquipmentQuery)) *OrderQuery {
	query := &EquipmentQuery{config: oq.config}
	for _, opt := range opts {
		opt(query)
	}
	oq.withEquipment = query
	return oq
}
...
func (oq *OrderQuery) sqlAll(ctx context.Context) ([]*Order, error) {
	var (
		...
		_spec       = oq.querySpec()
		loadedTypes = [3]bool{
			oq.withEquipment != nil,
			oq.withCompany != nil,
			oq.withUser != nil,
		}
	)
	if oq.withEquipment != nil || oq.withCompany != nil || oq.withUser != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := oq.withEquipment; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Order)
		for i := range nodes {
			if fk := nodes[i].equipment_order_equipmentt; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "equipment_order_equipmentt" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := oq.withCompany; query != nil {
		...
		nodeids := make(map[int][]*Order)
		for i := range nodes {
			if fk := nodes[i].company_order_companyy; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "company_order_companyy" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := oq.withUser; query != nil {
		...
		nodeids := make(map[int][]*Order)
		for i := range nodes {
			if fk := nodes[i].user_order_userr; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_order_userr" returned %v`, n.ID)
			}
			for i := range nodes {
