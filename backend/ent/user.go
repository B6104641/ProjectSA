...
// OrderUserrOrErr returns the OrderUserr value or an error if the edge
// was not loaded in eager-loading.
func (e UserEdges) OrderUserrOrErr() ([]*Order, error) {
	if e.loadedTypes[0] {
		return e.OrderUserr, nil
	}
	return nil, &NotLoadedError{edge: "order_userr"}
}
...
func (u *User) assignValues(values ...interface{}) error {
	...
	if value, ok := values[1].(*sql.NullString); !ok {
		return fmt.Errorf("unexpected type %T for field USER_PASSWORD", values[1])
	} else if value.Valid {
...
// QueryOrderUserr queries the order_userr edge of the User.
func (u *User) QueryOrderUserr() *OrderQuery {
	return (&UserClient{config: u.config}).QueryOrderUserr(u)
}
